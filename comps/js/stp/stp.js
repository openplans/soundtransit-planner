...
/**
 * Namespace: stp (A place for all things soundtransit planner-y.)
 * stp namespace controller / Class wrapper functionality
 * @class
 */
var stp = {
    ...
    /**
     * Function: namespace
     * Create a namespace given a string, eg: stp.namespace("stp.widget.ui");
     *
     * Parameters:
    ...
    namespace : function(ns, context) 
    {
        ...
        for(var i=0; i<num; ++i)
        {
            ...
            if(parts.length > 0) {
                stp.namespace([parts.join('.')], context[base]);
            }
        }
    },
     ...
     * 
     * Class {
     *   myFunction : stp.defaultMethod,
     * }
     * in the above...if this.myFucntion() is not overwritten by customer, I will tell you about it via the defaultMethod.
     */
    defaultMethod : function()
    { 
        console.log("NOTE: you're calling a stp.defaultMethod() - please make sure all logic is implemented.");
    },
...
// IMPORTANT: stp.Class & stp.extend was 'borrowed' from OpenLayers.Class & OpenLayers.Util.extend.  
...
/**
 * Constructor: stp.Class
 * Base class used to construct all other classes. Includes support for 
 *     multiple inheritance. 
 * 
 * To create a new stp-style class, use the following syntax:
 * > var MyClass = stp.Class(prototype);
 *
 * To create a new stp-style class with multiple inheritance, use the
 *     following syntax:
 * > var MyClass = stp.Class(Class1, Class2, prototype);
 ...
 */
stp.extend = function(destination, source) {
 ...
 */
stp.override = function(destination, source) {
    destination = destination || {};
    if(source) 
    {
        for(var property in source) 
 ...
 */
stp.inherit = function(destination, source) {
    destination = destination || {};
    if(source) 
    {
        for(var property in source) 
 ...
 */
stp.configure = function(destination, source, getAll) {
...
stp.clone = function(obj)
{
    ...
    for(var key in obj)
    {
        if (copied[obj[key]])
          ...
          temp[key] = stp.clone(obj[key], copied);
...
/** private method */
stp._copyProp = function(property, destination, value)
{
